...
function zoomElement() {
    ...
    var elementList = $("#content").find("div")
    elementList.push($("#content").find("img"))
    elementList.push($("#content").find("button"))
    elementList.each((i, item) => {
        console.log($(item).css("border-bottom"))
        $(item).css({
            "height": $(item).css("height").split("px")[0] * r + "px",
            "width": $(item).css("width").split("px")[0] * r + "px",
            "padding-top": $(item).css("padding-top").split("px")[0] * r + "px",
            "margin-top": $(item).css("margin-top").split("px")[0] * r + "px",
            "margin-left": $(item).css("margin-left").split("px")[0] * r + "px",
            "font-size": $(item).css("font-size").split("px")[0] * r + "px",
        })
    })
    $("#content").find("input").each((i, item) => {
        $(item).css({
            "height": $(item).css("height").split("px")[0] * r + "px",
            "width": ($(item).css("width").split("px")[0]) * r + "px",
            "padding-top": $(item).css("padding-top").split("px")[0] * r + "px",
...
function hidec(v) {
    if (getId("province").value != "请选择省份") {//如果先点击城市下拉框，则将省份的input的字被换成了#FF0000
        getId(v).style.display = getId(v).style.display == "none" ? "" : "none";
    } else {
